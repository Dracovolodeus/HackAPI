...
from crud.user.add_want_team import add_want_user_for_team
...
@router.post(f"{settings.api.add}{settings.api.want_the_user}", status_code=status.HTTP_200_OK)
async def add_to_want_the_user(
    ...
    try:
        await add_want_user_for_team(session=session, user_id=user_id, team_id=team_id)
    except DuplicateError:
        raise HTTPException(
            status_code=status.HTTP_304_NOT_MODIFIED, detail="already added"
        )
