...
@router.post(
    f"{settings.api.create}{settings.api.user}",
...
async def create_user(
    ...
    return await users_crud.create_any_user(
        any_user_create=user_create,
        session=session,
        role_id=settings.role.user,
    )


@router.get(
    f"{settings.api.get}{settings.api.user}",
    response_model=UserRead,
...
async def get_user(
    ...
    if not (
        ...
        or await verify_is_i(caller_user=caller_user, user_id=user_id)
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not enough rights error"
        )
    user = await users_crud.get_any_user(
        session=session, any_user_id=user_id, role_id=settings.role.user
    )
    return user
...
async def full_right_update_user(
    ...
    if not (
        ...
        or await verify_is_i(caller_user=caller_user, user_id=user_update.id)
    ):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not enough rights error"
        )
...
async def part_right_update_user(
    session: Annotated[AsyncSession, Depends(db_helper.session_getter)],
    user_update: UserPartUpdate,
    caller_user: Annotated[User, Depends(get_current_user_from_access_token)],
):
    if not (
        await verify_token_user_identity(
...
async def self_update(
    ...
    user_update = UserPartUpdate(**user_self_update.model_dump(), id=caller_user.id)
    if not await verify_is_i(caller_user=caller_user, user_id=user_update.id):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN, detail="Not enough rights error"
        )
    return await users_crud.update_any_user(
...
@router.delete(f"{settings.api.delete}{settings.api.user}")
